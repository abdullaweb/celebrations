        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Update Invoice </h4><br><br>
                            <div class="row">
                                <div class="col-md-1">
                                    <div class="md-3">
                                        ...
                                        <input class="form-control" type="text" name="invoice_no"
                                            value="{{ $invoiceInfo->invoice_no }}" id="invoice_no" readonly
                                ...
                                <div class="col-md-2">
                                    <div class="md-3">
                                        ...
                                        <input type="date" value="{{ $invoiceInfo->date }}" class="form-control"
                                            name="date" id="date">
                                    </div>
                                </div>
                            ...
                            <div class="row mt-2">
                                <div class="col-md-2">
                                    <div class="md-3">
                                        <label for="example-text-input" class="col-sm-12 col-form-label">Bill Number</label>
                                        <input type="text" class="form-control" name="po_number" id="po_number"
                                            placeholder="Enter Bill Number" value="{{ $invoiceInfo->po_number }}" required>
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="md-3">
                                        ...
                                        <select name="company_id" id="company_id" class="form-control form-select select2">
                                            ...
                                            @foreach ($companies as $company)
                                                <option value="{{ $company->id }}"
                                                    {{ $company->id == $invoiceInfo->company_id ? 'selected' : '' }}>
                                                    {{ $company->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="sub_cat_id" id="sub_cat_id" class="form-control form-select select2">
                                            ...
                                            {{-- @foreach ($subCategories as $sub_category)
                                                <option value="{{ $sub_category->id }}"
                                                    {{ $sub_category->category_id == $invoiceDetails->category_id ? 'selected' : '' }}>
                                                    {{ $sub_category->name }}
                                                </option>
                                            @endforeach --}}
                                        </select>
                            ...
                            <form method="POST" action="{{ route('invoice.update') }}" novalidate="" id="invoiceForm"
                                            ...
                                            <tr class="delete_add_more_item" id="delete_add_more_item">
                                                ...
                                                <td width="20%">
                                                    <input type="hidden" name="sub_cat_id[]"
                                                        value="{{ $details->sub_cat_id }}" readonly>
                                                    <span class="sub_cat_name">{{ $details->sub_category->name }}</span>
                                                </td>
                                                <td>
                                                    <input type="text" class="form-control"
                                                        placeholder="Write Description" name="description[]"
                                                        autocomplete="off" value="{{ $details->description }}">
                                                </td>
                                                <td width="8%">
                                                    <input type="text" class="form-control size" id="size"
                                                        placeholder="Size" name="size[]" autocomplete="off"
                                                        value="{{ $details->size }}">
                                                </td>
                                                <td width="4%">
                                                    <input type="digit" class="form-control selling_qty text-right"
                                                        required="" data-parsley-required-message="Qty is required"
                                                        name="selling_qty[]" value="{{ $details->selling_qty }}"
                                                        autocomplete="off">
                                                </td>
                                                <td width="10%">
                                                ...
                                                <td>
                                                    <input type="digit" class="form-control selling_price text-right"
                                                        ...
                                                        name="selling_price[]" value="{{ $details->selling_price }}"
                                                        readonly>
                                                </td>
                                                <td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" name="estimated_amount" id="estimated_amount"
                                                    ...
                                                    readonly value="{{ $payment->total_amount }}">
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-3" id="vat_tax_col">
                                        <select name="vat_tax_field" id="vat_tax_field" class="form-control" required data-parsley-required-message="Vat field is required">
                                            <option value="" selected disabled>Select Vat Tax</option>
                                            @foreach ($taxes as $tax)
                                                <option value="{{ $tax->id }}"
                                                    {{ $payment->vat_tax == $tax->rate ? 'selected' : '' }}>
                                                    {{ $tax->name }}</option>
                                            @endforeach
                                            <option value="0" {{ $payment->vat_tax == 0 ? 'selected' : '' }}>Without
                                                Vat/Tax</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <select class="form-control" name="paid_status" id="paid_status" required data-parsley-required-message="Paid Status is required">
                                ...
                                <div class="form-group mt-4">
                                    <button type="submit" class="btn btn-info" id="storeButton">Invoice Update</button>
                                </div>
                            </form>
                        </div>
                    </div>
